...
if ( !empty( $status_terms ) && !is_wp_error( $status_terms ) ) {
    ...
    <div id="gallery-items-filter" class="gallery-items-filter clearfix">
        <a href="#" class="active" data-filter="*"><?php esc_html_e( 'All', 'inspiry' ); ?></a>
        <?php
			...
			<div class="col-xs-6 col-gallery-item isotope-item <?php echo esc_attr( $property_status_based_classes ); echo esc_attr( $property_class ); ?>">
				...
				<article class="gallery-post-item gallery-listing-post clearfix hentry">
					...
					<figure class="gallery-thumbnail">
						<div class="media-container">
							<div class="btn-wrapper">
								...
								if ( $full_image_url ) {
									?><a class="search-plus swipebox" href="<?php echo esc_url( $full_image_url ); ?>" title="<?php the_title_attribute(); ?>"><i class="fa fa-search-plus"></i></a><?php
								}
								?>
								<a class="external-link" href="<?php the_permalink(); ?>"><i class="fas fa-external-link-alt"></i></a>
							</div>
						</div>
						<?php
					...
					<div class="gallery-title-wrapper">
						<h3 class="gallery-item-title entry-title"><a href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>" rel="bookmark"><?php the_title(); ?></a></h3>
					</div>
